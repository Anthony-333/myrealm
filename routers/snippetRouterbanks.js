...
const Snippetbanks = require("../models/snippetModel-banks");
const auth = require("../middleware/auth");

router.get("/", auth, async (req, res) => {
  try {
    const snippets = await Snippetbanks.find({ user: req.user });
    res.json(snippets);
  } catch (err) {
    res.status(500).send();
  }
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const newSnippet = new Snippetbanks({
      title,
      description,
      code,
      user: req.user,
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    const originalSnippet = await Snippetbanks.findById(snippetId);
    if (!originalSnippet)
      return res.status(400).json({
        errorMessage:
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    const existingSnippet = await Snippetbanks.findById(snippetId);
    if (!existingSnippet)
      return res.status(400).json({
        errorMessage:
          "No snippet with this ID was found. Please contact the developer.",
