...
const Snippetinsurances = require("../models/snippetModel-insurances");
const auth = require("../middleware/auth");

router.get("/", auth, async (req, res) => {
  try {
    const snippets = await Snippetinsurances.find({ user: req.user });
    res.json(snippets);
  } catch (err) {
    res.status(500).send();
  }
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const newSnippet = new Snippetinsurances({
      title,
      description,
      code,
      user: req.user,
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    const originalSnippet = await Snippetinsurances.findById(snippetId);
    if (!originalSnippet)
      return res.status(400).json({
        errorMessage:
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    const existingSnippet = await Snippetinsurances.findById(snippetId);
    if (!existingSnippet)
      return res.status(400).json({
        errorMessage:
          "No snippet with this ID was found. Please contact the developer.",
